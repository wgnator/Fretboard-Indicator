...
function init() {
  ...
  for (i = 0; i < 6; i++) {
    ...
    stringHTML += `<div class='string no_${
      6 - i
    }' style='border-width:${stringThickness}px'></div>\n`;
  }
  ...
  document.querySelector('.lower_side').scrollLeft =
    neckCanvas.offsetWidth - window.innerWidth - guitarHead.offsetWidth + 50;
}
...
sequenceNameInput.onkeydown = () => {
  inputTextResize(sequenceNameInput);
};
...
function noteClicked(clickedLabelTag) {
  ...
  if (notes[noteName.noteName].readyState === 4) notes[noteName.noteName].play();
  noteNameAnimation(noteName.noteName, clickedLabelTag);
  if (noteMode == 'chord') {
...
function saveSequence() {
  ...
  seq.innerText = `sequence ${sequenceNum}: ${
    sequenceName ? sequenceName : noteMode == 'line' ? 'line' : 'chord'
  }`;
...
function replaySequence(sequenceNum) {
  console.log(
    `sequence ${sequenceNum}: ${savedSequence[sequenceNum - 1].sequence.map((e) => e[2])}`,
